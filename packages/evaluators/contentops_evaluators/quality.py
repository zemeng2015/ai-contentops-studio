...
class HeuristicContentEvaluator:
    ...
    def evaluate(self, packet: ResearchPacket, draft: Draft) -> EvaluationReport:
        ...
        source_quality = sum(source.extraction_quality for source in packet.sources) / max(
            len(packet.sources), 1
        )
        grounded_claims = sum(
        ...
        if source_quality < self.min_publish_score:
            findings.append("Improve source extraction quality before publishing.")
