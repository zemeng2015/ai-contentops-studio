  ...
  container_definitions = jsonencode([
    {
      ...
      command = concat(
        [
          ...
          "--json",
          "--window-size",
          tostring(var.release_gate_window_size)
        ],
        var.release_gate_require_approval ? [] : ["--no-require-approval"]
      )
      environment = [
        { name = "CONTENTOPS_ARTIFACT_STORE_PROVIDER", value = "s3" },
        { name = "CONTENTOPS_ARTIFACT_S3_BUCKET", value = aws_s3_bucket.artifacts.bucket },
      ...
      logConfiguration = {
        ...
        options = {
          awslogs-group         = aws_cloudwatch_log_group.worker.name
          awslogs-region        = var.aws_region
          awslogs-stream-prefix = "release-gate"
        }
      }
    }
  ])
...
resource "aws_scheduler_schedule" "release_gate" {
  name                         = "${local.name}-release-gate"
  group_name                   = aws_scheduler_schedule_group.contentops.name
  schedule_expression          = var.release_gate_schedule_expression
  schedule_expression_timezone = var.worker_schedule_timezone
  state                        = var.release_gate_schedule_enabled ? "ENABLED" : "DISABLED"
    ...
    widgets = [
      {
        ...
        height = 2
        properties = {
          markdown = "# AI ContentOps Studio Operations\nRelease evidence: `/release-evidence`; release gates: `/release-gates`; worker receipts: `/job-executions`; worker alerts: `/job-executions/alerts`; alert deliveries: `/job-executions/alerts/notifications`."
        }
      },
      {
        type   = "metric"
