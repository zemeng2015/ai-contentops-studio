...
Persist a report under the default artifact root and inspect recent history:

```powershell
contentops integration-smoke-run --selector feed --dry-run --record --json
contentops integration-smoke-runs --json
```
